...
export class AccountServiceProxy {
     ...
     * @return Success
     */
    sendPasswordResetCode(body: ForgotPassword | undefined): Observable<ForgotPassword> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<ForgotPassword>><any>_observableThrow(e);
                }
            } else
                return <Observable<ForgotPassword>><any>_observableThrow(response_);
        }));
    }

    protected processSendPasswordResetCode(response: HttpResponseBase): Observable<ForgotPassword> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = ForgotPassword.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
     ...
     * @return Success
     */
    resetPassword(body: ResetPassword | undefined): Observable<void> {
...
export class ForgotPassword implements IForgotPassword {
    ...
    init(_data?: any) {
        if (_data) {
            this.emailAddress = _data["emailAddress"];
            this.sentSuccessfully = _data["sentSuccessfully"];
        }
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        data["emailAddress"] = this.emailAddress;
        data["sentSuccessfully"] = this.sentSuccessfully;
        return data; 
    }

    clone(): ForgotPassword {
        const json = this.toJSON();
        let result = new ForgotPassword();
        result.init(json);
        return result;
